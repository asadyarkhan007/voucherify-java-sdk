  ...
  private void start() {
    ...
    client.vouchers().rx()
        ...
        .map(voucherResponse -> {
          ...
          return client.vouchers().update(voucherResponse.getCode(), voucherUpdate);
        })
        .doOnNext(voucherResponse -> System.out.println("Voucher updated. Metadata: " + voucherResponse.getMetadata()))
        .map(voucherResponse -> {
          ...
          return client.redemptions().redeem(voucherResponse.getCode(), redeemVoucher);
        })
        .doOnNext(redeemVoucherResponse -> System.out.println("Voucher redeemed. RedemptionId: " + redeemVoucherResponse.getId()))
        .delay(5, TimeUnit.SECONDS)
        .map(redeemVoucherResponse -> {
          Customer customer = Customer.builder().sourceId("alice.morgan").build();
          RollbackRedemption rollbackRedemption = RollbackRedemption.builder().customer(customer).build();
          return client.redemptions().rollback(redeemVoucherResponse.getId(), "test", rollbackRedemption);
        })
        .doOnNext(rollbackRedemptionResponse -> System.out.println("Redemption rolled-back. RedemptionId: " + rollbackRedemptionResponse.getId()))
        .doOnError(throwable -> System.out.println("Something went wrong. Error: " + throwable))
        .blockingLast();
  }
